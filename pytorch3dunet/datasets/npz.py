...
#        test_patient = os.listdir(os.path.join(file_path, 'test'))
        ...
        image_sample = resize(image_sample.astype(np.float32), (128,128,128), anti_aliasing = False)
        image_sample = np.expand_dims(image_sample, 0)
    ...
    def getImage(self, count):
        ...
        self.cur_image = resize(self.cur_image.astype(np.float32), (128, 128, 128), anti_aliasing = False)
...
#                self.cur_mask = self.cur_mask[100:-100, :, :]
            self.cur_mask = resize(self.cur_mask.astype(np.float32), (128, 128, 128), anti_aliasing = False)
