...
class MlpInner(nn.Module):
    def __init__(self, embed_dim, cube_size, patch_size):
        super().__init__()
...
def makeReversibleSequence(embed_dim, num_heads, dropout, cube_size, patch_size):
    # innerChannels = channels // 2
    # groups = CHANNELS[0] // 2
    fBlock = AttentionInner(embed_dim, num_heads, dropout)
    gBlock = MlpInner(embed_dim, cube_size, patch_size)
...
class Transformer(nn.Module):
    def __init__(self, input_dim, embed_dim, cube_size, patch_size, num_heads, num_layers, dropout, extract_layers):
        ...
        for _ in range(num_layers):
            layer = makeReversibleSequence(embed_dim, num_heads, dropout, cube_size, patch_size)
...
class RevUNETR(nn.Module):
    def __init__(self, img_shape=(128, 128, 128), input_dim=4, output_dim=3, embed_dim=768, patch_size=16, num_heads=12, dropout=0.1, **kwargs):
        super().__init__()
        global CHANNELS
