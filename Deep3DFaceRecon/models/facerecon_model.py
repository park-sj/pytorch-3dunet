...
from .innofitUV import ParametricFaceModel
...
class FaceReconModel(BaseModel):
    ...
    def forward(self):
        print("input", self.input_img, torch.max(self.input_img), torch.min(self.input_img))
        output_coeff = self.net_recon(self.input_img)
        print("output_coef", output_coeff)
        self.facemodel.to(self.device)
        self.mesh_rasterizer.to(self.device)
        self.pred_vertex, self.pred_tex, self.pred_color, self.pred_lm = \
            self.facemodel.compute_for_render(output_coeff)
        # self.pred_color # interpolate on pixels
        self.pred_mask, _, self.pred_face = self.renderer(
            self.mesh_rasterizer, self.pred_vertex, 
            self.facemodel.face_buf, textures=self.pred_tex, gamma=self.pred_color
            )
    ...
    def compute_losses(self):
        ...
        if not self.opt.use_predef_M:
            print(self.pred_lm.shape, self.input_img.shape[-2], self.image_paths)
            print(self.pred_lm)
    ...
    def save_mesh(self, name):
        ...
        recon_shape[..., 0] = 10 - recon_shape[..., 0] # from camera space to world space
        recon_shape = recon_shape.cpu().numpy()[0]
        recon_color = self.pred_color
        recon_color = recon_color.cpu().numpy()[0]
        tri = self.facemodel.face_buf.cpu().numpy()
        uvtri = self.facemodel.uvtri.cpu().numpy()
        vt = self.facemodel.vt.cpu().numpy()
        texture = np.flip(self.pred_tex.maps_padded().squeeze().cpu().numpy(), 2)
        self.save_obj(vertices=recon_shape, faces=tri, vt=vt, uvtri=uvtri, name=name)
        self.save_mtl(name)
        print(np.max(texture), np.min(texture))
        self.save_png(np.clip(100. * texture, 0, 255).astype(np.uint8), name)

    @staticmethod
    def save_obj(vertices, faces, vt, uvtri, name):
        basename = os.path.splitext(name)[0]
        with open(name, "w") as f:
            f.write(f"mtllib {os.path.basename(basename)}.mtl\n")
            for i in range(len(vertices)):
                f.write(f"v {vertices[i][0]} {vertices[i][1]} {vertices[i][2]}\n")
            for i in range(len(vt)):
                f.write(f"vt {vt[i][0]} {vt[i][1]}\n")
            for i, face in enumerate(faces):
                f.write(f"f {face[0]+1}/{uvtri[i][0]+1} {face[1]+1}/{uvtri[i][1]+1} {face[2]+1}/{uvtri[i][2]+1}\n")
    ...
    def save_mtl(name):
        ...
        with open(basename + ".mtl", "w") as f:
            ...
            f.write("Ks 1 1 1\n")
            f.write("Ns 1000\n")
            f.write(f"map_Kd {os.path.basename(basename)}_1.png")
    
    @staticmethod
    def save_png(texture, name):
        print(texture.shape)
        basename = os.path.splitext(name)[0]
        cv2.imwrite(basename + "_1.png", texture)
